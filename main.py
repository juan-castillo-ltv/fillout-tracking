...
from config import ICU_USER_LIST, TFX_GOOGLE_ADS_ID, TFX_USER_LIST, COD_GOOGLE_ADS_ID, COD_USER_LIST, APPS_CONFIG
from config import TFX_META_APP_ID, TFX_META_APP_SECRET, TFX_META_LONG_LIVED_TOKEN, TFX_META_AD_ACCOUNT_ID, TFX_META_CUSTOM_AUDIENCE_ID, BREVO_API_TOKEN

brevo_api_key = BREVO_API_TOKEN
...
def track_new_pc_user():
    ...
    shop_url_contact = event_data.get('data', {}).get('item', {}).get('custom_attributes',{}).get('shop_url'), # Update this depending on the app
    payload = {
                "email": event_data.get('data', {}).get('item', {}).get('email'),
                "ext_id": event_data.get('data', {}).get('item', {}).get('id'),
                "attributes": {
                    "SHOP_NAME": event_data.get('data', {}).get('item', {}).get('name'),
                    "APP": app_name, # Update this depending on the app
                    "ACTIVE": True,
                ...
                "updateEnabled": True,
                "listIds": [int(APPS_CONFIG[0]["brevo_active_list"])] # Update this depending on the app
            }
    ...
    try:
        ...
        logging.info(f"Contacts successfully added to list ID {APPS_CONFIG[0]['brevo_active_list']}.")
    except requests.exceptions.RequestException as e:
        logging.error(f"Failed to add contacts. Error: {e}. Response: {response.text}")       

    # Add contact to Google Ads List
...
def track_new_icu_user():
    ...
    payload = {
                ...
                "attributes": {
                    ...
                    "DAYS_UNINSTALLED": 0,
                    "DAYS_CLOSED": 0,
                    "TRIAL_DAYS_REMAINING": 30, # Update this depending on the app
                    "DAYS_SINCE_BILLED": None,
                    "PAID_ACTIVE": False,
                    "COUNTRY": event_data.get('data', {}).get('item', {}).get('location',{}).get('country'),
                    "REGION": event_data.get('data', {}).get('item', {}).get('location',{}).get('region'),
                ...
                "updateEnabled": True,
                "listIds": [int(APPS_CONFIG[1]["brevo_active_list"])] # Update this depending on the app
            }
    # Add the correct attributes based on the `app` value
    if app_name == 'PC':
        payload["attributes"].update({
    ...
    try:
        ...
        #logging.info(json.dumps(payload, indent=4))
        response.raise_for_status()  # Raises an HTTPError for bad responses
        logging.info(f"Contacts successfully added to list ID {APPS_CONFIG[1]['brevo_active_list']}.")
    except requests.exceptions.RequestException as e:
        logging.error(f"Failed to add contacts. Error: {e}. Response: {response.text}")       
...
def track_new_tfx_user():
    ...
    #Add contact to Brevo Active User List
    shop_url_contact = event_data.get('data', {}).get('item', {}).get('custom_attributes',{}).get('shopify_url'), # Update this depending on the app
    payload = {
                "email": event_data.get('data', {}).get('item', {}).get('email'),
                "ext_id": event_data.get('data', {}).get('item', {}).get('id'),
                "attributes": {
                    ...
                    "ACTIVE": True,
                    "SHOP_URL": shop_url_contact,
                    "SHOPIFY_PLAN": event_data.get('data', {}).get('item', {}).get('custom_attributes',{}).get('plan_display_name'), # Update this depending on the app
                    "INSTALLED_AT": datetime.datetime.fromisoformat(event_data.get('data', {}).get('item', {}).get('created_at')).date().strftime("%Y-%m-%d"),
                    "UNINSTALLED_AT": None,
                    "LAST_SUBSCRIPTION_CHARGED_AT": None,
                    "NUMBER_OF_SUBSCRIPTION_CHARGES": None,
                    ...
                    "DAYS_UNINSTALLED": 0,
                    "DAYS_CLOSED": 0,
                    "TRIAL_DAYS_REMAINING": 7, # Update this depending on the app
                    "DAYS_SINCE_BILLED": None,
                    "PAID_ACTIVE": False,
                    "COUNTRY": event_data.get('data', {}).get('item', {}).get('location',{}).get('country'),
                    "REGION": event_data.get('data', {}).get('item', {}).get('location',{}).get('region'),
                    "CITY": event_data.get('data', {}).get('item', {}).get('location',{}).get('city'),
                    "PLAN": event_data.get('data', {}).get('item', {}).get('custom_attributes',{}).get('plan_display_name'), # Update this depending on the app,
                    "TIME_CLOSED": None,
                    "TIME_UNINSTALLED": None,
                    "SIGNED_UP": datetime.datetime.fromisoformat(event_data.get('data', {}).get('item', {}).get('created_at')).date().strftime("%Y-%m-%d"),
                    "SHOPIFY_URL_RAW": shop_url_contact[0].split('.myshopify.com')[0] if '.myshopify.com' in shop_url_contact[0] else shop_url_contact[0],
                ...
                "updateEnabled": True,
                "listIds": [int(APPS_CONFIG[2]["brevo_active_list"])] # Update this depending on the app
            }
    # Add the correct attributes based on the `app` value
    if app_name == 'PC':
        payload["attributes"].update({
    ...
    try:
        ...
        #logging.info(json.dumps(payload, indent=4))
        response.raise_for_status()  # Raises an HTTPError for bad responses
        logging.info(f"Contacts successfully added to list ID {APPS_CONFIG[2]['brevo_active_list']}.")
    except requests.exceptions.RequestException as e:
        logging.error(f"Failed to add contacts. Error: {e}. Response: {response.text}")   
